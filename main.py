"""
    project: project_3
    author: Robert Frdlík
    email: puchmeloun@gmail.com
    discord: puch_frog
...
def krok1(url, output_file): #stahne obsah stranky
    response = requests.get(url)
    if response.status_code == 200:
        soup = BeautifulSoup(response.content, 'html.parser')
        rows = soup.find_all('tr') #najde radky
        
        data = []  # List to store all rows of data

        for row in rows:
            cells = row.find_all("td")#pro kazdy radek najde bunky
            if len(cells) >= 2:#pokud ma radek 2 a vice bunek tak je zpracuje
                cell1 = cells.pop(0)
                cell2 = cells.pop(0)
        ...
        with open(output_file, 'w', newline='', encoding='cp1250') as file: #encoding prelozi do cestiny :D
...
def krok2(url, excel_rows, row_data):
    response = requests.get(url)  #stahne obsah stranky, status 200 znamena ze se to povedlo YIPPEE
    #row_data = []
    if response.status_code == 200:
        soup = BeautifulSoup(response.content, 'html.parser')
        rows = soup.find_all('tr')#najde radky
